-- PO draft edits are server-only. A sent/partially received/received PO is
-- immutable so scanned receipts always reconcile to the supplier order.
create or replace function public.update_v2_purchase_order_with_details(
...
begin
  ...
  if v_order.status <> 'draft' then raise exception 'Only a draft purchase order can be edited'; end if;
  ...
  delete from public.purchase_order_lines where purchase_order_id = v_order.id;
  ...
  values (
    ...
    'Updated detailed draft purchase order', 'success',
  ...
  return jsonb_build_object('alreadyUpdated', false, 'id', v_order.id, 'purchaseOrderNumber', v_order.purchase_order_number, 'status', 'draft');
end $$;

revoke all on function public.update_v2_purchase_order_with_details(bigint, text, text, bigint, date, date, numeric, jsonb, text, text, bigint, text) from public, anon, authenticated;
grant execute on function public.update_v2_purchase_order_with_details(bigint, text, text, bigint, date, date, numeric, jsonb, text, text, bigint, text) to service_role;
