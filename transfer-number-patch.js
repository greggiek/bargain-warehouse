(() => {
  ...
  const transferStatusLabel=status=>({draft:'Draft',allocated:'Allocated',in_transit:'Shipped / In Transit',partially_received:'Partially Received',completed:'Completed',canceled:'Canceled',awaiting_receipt:'Shipped / In Transit',receiving:'Receiving',problem:'Needs Review'}[status]||String(status||'').replaceAll('_',' '));
  ...
  async function loadMasterTransferBucket(){
    ...
    try{
      ...
      host.innerHTML=`<div class="section-head"><div><div class="eyebrow">ALL LOCATIONS</div><h2>Master Transfer Bucket</h2><p class="muted">Every active transfer across the company in one queue.</p></div><strong id="masterTransferCount">${rows.length} active</strong></div><div class="master-transfer-filters"><input id="masterTransferSearch" placeholder="Search transfer #, warehouse, or user"><select id="masterTransferStatus"><option value="">All statuses</option>${[...new Set(rows.map(row=>row.status).filter(Boolean))].sort().map(status=>`<option value="${esc(status)}">${esc(status.replaceAll('_',' '))}</option>`).join('')}</select><select id="masterTransferLocation"><option value="">All warehouses</option>${locations.map(location=>`<option value="${esc(location)}">${esc(location)}</option>`).join('')}</select></div><div class="master-transfer-table-wrap"><table class="master-transfer-table"><thead><tr><th>Transfer</th><th>From</th><th>To</th><th>Status</th><th>Lines</th><th>Pieces</th><th>Actions</th></tr></thead><tbody id="masterTransferRows"></tbody></table></div>`;
      const render=()=>{const search=host.querySelector('#masterTransferSearch').value.trim().toLowerCase(),status=host.querySelector('#masterTransferStatus').value,location=host.querySelector('#masterTransferLocation').value,shown=rows.filter(row=>(!search||`${row.ref} ${row.from} ${row.to} ${row.createdBy}`.toLowerCase().includes(search))&&(!status||row.status===status)&&(!location||row.from===location||row.to===location));host.querySelector('#masterTransferCount').textContent=`${shown.length} of ${rows.length} active`;host.querySelector('#masterTransferRows').innerHTML=shown.map(row=>{const edit=['draft','allocated'].includes(row.status),receive=['in_transit','partially_received','awaiting_receipt','receiving'].includes(row.status);return `<tr><td><strong>${esc(row.ref)}</strong><small>${esc(row.createdBy||'')}</small></td><td>${esc(row.from)}</td><td>${esc(row.to)}</td><td><span class="master-status ${esc(row.status)}">${esc(transferStatusLabel(row.status))}</span></td><td>${row.lines?.length||0}</td><td>${(row.lines||[]).reduce((sum,line)=>sum+Number(line.expected||0),0)}</td><td><div class="master-doc-actions">${edit?`<button data-edit-transfer="${row.id}">Edit</button>`:''}${row.status==='draft'?`<button data-transfer-action="allocate" data-transfer-id="${row.id}">Allocate</button><button class="danger-lite" data-delete-transfer="${row.id}">Delete</button>`:''}${row.status==='allocated'?`<button class="primary-action" data-transfer-action="ship" data-transfer-id="${row.id}">Ship</button><button class="danger-lite" data-transfer-action="cancel" data-transfer-id="${row.id}">Cancel</button>`:''}${receive?`<button class="primary-action" data-transfer-action="receive" data-transfer-id="${row.id}">Receive</button>`:''}</div></td></tr>`}).join('')||'<tr><td colspan="7" class="muted">No transfers match these filters.</td></tr>'};
      ['masterTransferSearch','masterTransferStatus','masterTransferLocation'].forEach(id=>host.querySelector('#'+id).addEventListener(id==='masterTransferSearch'?'input':'change',render));host.addEventListener('click',async event=>{const edit=event.target.closest('[data-edit-transfer]'),remove=event.target.closest('[data-delete-transfer]'),action=event.target.closest('[data-transfer-action]');if(!edit&&!remove&&!action)return;const id=(edit||remove)?.dataset[edit?'editTransfer':'deleteTransfer']||action?.dataset.transferId,row=rows.find(item=>String(item.id)===String(id));if(!row)return;const operation=remove?'delete':edit?'update':action.dataset.transferAction;if(operation==='receive'){backendTransfers[row.ref]=row;go('transferCheck');setTimeout(()=>showTransferCheck(row.ref),0);return}if(['delete','cancel'].includes(operation)&&!confirm(`${operation==='delete'?'Delete':'Cancel'} ${row.ref}?`))return;if(operation==='ship'&&!confirm(`Ship ${row.ref}? Material will be marked In Transit.`))return;const body={id:row.id,operation};if(edit){body.notes=prompt('Transfer note:',row.note||'');if(body.notes===null)return;body.lines=[];for(const line of row.lines||[]){const qty=prompt(`${line.sku} quantity:`,line.expected);if(qty===null)return;body.lines.push({id:line.id,qty:Number(qty)})}}const button=edit||remove||action;button.disabled=true;try{const token=await window.bmGoogleAuth.accessToken(),response=await fetch('/api/warehouse?action=manage-transfer',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify(body)}),data=await response.json();if(!response.ok)throw new Error(data.error||'Could not update transfer');notify(operation==='delete'?'Transfer deleted':operation==='allocate'?'Material allocated':operation==='ship'?'Transfer shipped':operation==='cancel'?'Transfer canceled':'Transfer updated');host.remove();loadMasterTransferBucket()}catch(error){notify(error.message);button.disabled=false}});render();
  ...
  renderTransferCreate = function () {
    ...
    if(finish){
      finish.textContent='Allocate & Print Transfer';
  ...
  showCompletion = function (tx, kind) {
    ...
    if (kind === 'transfer' && pendingTransferNumber) {
      ...
      const transferToSave={transferNumber:tx.ref,from:tx.from,to:tx.to,note:tx.note||'',status:'allocated',lines:(tx.lines||[]).map(line=>({sku:line.sku,name:line.name,barcode:line.barcode||line.sku,qty:Number(line.qty||0)}))};
