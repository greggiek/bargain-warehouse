(() => {
  ...
  async function loadMasterTransferBucket(){
    ...
    try{
      ...
      const render=()=>{const search=host.querySelector('#masterTransferSearch').value.trim().toLowerCase(),status=host.querySelector('#masterTransferStatus').value,location=host.querySelector('#masterTransferLocation').value,shown=rows.filter(row=>(!search||`${row.ref} ${row.from} ${row.to} ${row.createdBy}`.toLowerCase().includes(search))&&(!status||row.status===status)&&(!location||row.from===location||row.to===location));host.querySelector('#masterTransferCount').textContent=`${shown.length} of ${rows.length} active`;host.querySelector('#masterTransferRows').innerHTML=shown.map(row=>`<tr><td><strong>${esc(row.ref)}</strong><small>${esc(row.createdBy||'')}</small></td><td>${esc(row.from)}</td><td>${esc(row.to)}</td><td><span class="master-status ${esc(row.status)}">${esc(String(row.status||'').replaceAll('_',' '))}</span></td><td>${row.lines?.length||0}</td><td>${(row.lines||[]).reduce((sum,line)=>sum+Number(line.expected||0),0)}</td><td><div class="master-doc-actions"><button data-edit-transfer="${row.id}">Edit</button><button class="danger-lite" data-delete-transfer="${row.id}">Delete</button></div></td></tr>`).join('')||'<tr><td colspan="7" class="muted">No transfers match these filters.</td></tr>'};
      ['masterTransferSearch','masterTransferStatus','masterTransferLocation'].forEach(id=>host.querySelector('#'+id).addEventListener(id==='masterTransferSearch'?'input':'change',render));host.addEventListener('click',async event=>{const edit=event.target.closest('[data-edit-transfer]'),remove=event.target.closest('[data-delete-transfer]');if(!edit&&!remove)return;const row=rows.find(item=>String(item.id)===String((edit||remove).dataset[edit?'editTransfer':'deleteTransfer']));if(!row)return;if(remove&&!confirm(`Delete ${row.ref}? This cannot be undone.`))return;const body={id:row.id,operation:remove?'delete':'update'};if(edit){body.notes=prompt('Transfer note:',row.note||'');if(body.notes===null)return;body.lines=[];for(const line of row.lines||[]){const qty=prompt(`${line.sku} quantity:`,line.expected);if(qty===null)return;body.lines.push({id:line.id,qty:Number(qty)})}}const button=edit||remove;button.disabled=true;try{const token=await window.bmGoogleAuth.accessToken(),response=await fetch('/api/warehouse?action=manage-transfer',{method:'POST',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify(body)}),data=await response.json();if(!response.ok)throw new Error(data.error||'Could not update transfer');notify(remove?'Transfer deleted':'Transfer updated');host.remove();loadMasterTransferBucket()}catch(error){notify(error.message);button.disabled=false}});render();
