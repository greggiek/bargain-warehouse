(() => {
  ...
  async function loadWaitingTransfers(){
    ...
    try{const token=await window.bmGoogleAuth.accessToken();if(!token)throw new Error('Your Google session expired. Refresh and sign in again.');const response=await fetch('/api/warehouse?action=waiting-transfers',{cache:'no-store',headers:{Authorization:`Bearer ${token}`}}),payload=await response.text();let data;try{data=JSON.parse(payload)}catch{throw new Error(response.ok?'The transfer response was invalid.':'The server could not load transfers.')}if(!response.ok)throw new Error(data.error||'Could not load transfers.');const selected=String(state.location||'').trim().toLowerCase(),transfers=(data.transfers||[]).filter(transfer=>String(transfer.to||'').trim().toLowerCase()===selected);for(const transfer of transfers)backendTransfers[transfer.ref]=transfer;host.innerHTML=`<div class="section-head"><div><div class="eyebrow">WAITING TO RECEIVE</div><h2>Open Transfers</h2><p class="muted">Transfers arriving at ${esc(state.location)}.</p></div><strong>${transfers.length} waiting</strong></div>${transfers.length?`<div class="waiting-transfer-list">${transfers.map(transfer=>`<button class="waiting-transfer" data-waiting-transfer="${esc(transfer.ref)}"><span><strong>${esc(transfer.ref)}</strong><small>${esc(transfer.from)} → ${esc(transfer.to)}</small></span><span>Receive ›</span></button>`).join('')}</div>`:'<p class="muted">No open transfers are waiting for this warehouse.</p>'}`;host.querySelectorAll('[data-waiting-transfer]').forEach(button=>button.onclick=()=>showTransferCheck(button.dataset.waitingTransfer))}catch(error){host.innerHTML=`<div class="eyebrow">WAITING TO RECEIVE</div><h2>Open Transfers</h2><p>${esc(error.message)}</p>`}
  }
