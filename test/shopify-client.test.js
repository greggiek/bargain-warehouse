...
test('rejects failed and malformed authentication responses', async () => {
  for (const authResponse of [response({ message: 'nope' }, false, 401), response(null)]) {
    const client = clientWith(async () => authResponse);
    await assert.rejects(client.access(stores[0]), /authentication failed/);
  }
});
...
test('rejects blank SKUs before making a request', async () => {
  let calls = 0;
  const client = clientWith(async () => {
    calls += 1;
    return response({ access_token: 'token' });
  });
  await assert.rejects(client.inventoryItemForSku('store_1', '  '), /SKU is required/);
...
test('rejects missing and duplicate exact SKU matches', async () => {
  for (const nodes of [[], [
    ...
    const client = clientWith(async url => url.endsWith('/admin/oauth/access_token')
...
test('rejects unknown stores', async () => {
  const client = clientWith(async () => response({ access_token: 'token' }));
  await assert.rejects(client.inventoryItemForSku('missing', 'SKU'), /Unknown Shopify store/);
});

test('reports Shopify HTTP and GraphQL errors', async () => {
  for (const graphqlResponse of [
    ...
    const client = clientWith(async url => url.endsWith('/admin/oauth/access_token')
      ...
      : graphqlResponse);
    await assert.rejects(
      client.inventoryItemForSku('store_1', 'SKU'),
      graphqlResponse.ok ? /Access denied/ : /SKU lookup failed \(503\)/
    );
  }
});

test('rejects a missing available quantity', async () => {
  const client = clientWith(async url => url.endsWith('/admin/oauth/access_token')
    ? response({ access_token: 'token', expires_in: 3600 })
    : response({ data: { inventoryItem: { inventoryLevel: { quantities: [] } } } }));
  await assert.rejects(
    client.availableQuantity('store_1', 'item', 'location'),
