(() => {
  ...
  const show = route => { expectedRoute = route; order = null; render(); $('salesOrderStepTwo').classList.add('receiving-next-step'); const will = route === 'will_call'; $('salesOrderKicker').textContent = will ? 'Will call' : 'Local delivery'; $('salesOrderTitle').textContent = will ? 'Pick will call' : 'Load local delivery'; $('salesOrderHelp').textContent = will ? 'Step 1 opens the Will Call pick ticket.' : 'Step 1 opens the Local Delivery loading ticket.'; $('salesOrderStepTwoTitle').textContent = will ? 'Pick items' : 'Load items'; $('salesOrderStepTwoHelp').textContent = will ? 'Opens automatically after Step 1.' : 'Opens automatically after Step 1.'; $('salesOrderScanInput').value = ''; set(''); if (!dialog.open) dialog.showModal(); $('salesOrderScanInput').focus(); };
  function render() {
    const host = $('salesOrderTicket'); if (!order) { host.hidden = true; return; } $('salesOrderStepTwo').classList.remove('receiving-next-step'); host.hidden = false;
