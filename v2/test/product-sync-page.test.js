...
test('product sync preview is wired into navigation', () => {
  assert.match(page, /id="productSyncNav"/);
  assert.match(page, /id="productSyncView"/);
  assert.match(page, /product-sync\.js/);
});

test('product sync interface enforces preview-only responses', () => {
...
test('product sync interface has no commit control', () => {
