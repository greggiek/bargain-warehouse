...
module.exports = async function shopifyTransferLifecycle(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const links = await postgrest(url, 'shopify_transfer_links?select=id,bm_reference,route_type,status,source_location_id,destination_location_id,source_store_key,destination_store_key,created_at,shipped_at,received_at,metadata,shopify_transfer_link_lines(sku,quantity)&order=created_at.desc&limit=50', 'GET', serviceRoleKey);
