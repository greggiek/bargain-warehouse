...
async function collectSales(store, startDate) {
  ...
  while (hasNextPage && page++ < 100) {
    const data = await graphql(shop, token, query, { cursor, query: `processed_at:>=${startDate} status:any` }); const connection = data.orders; if (!connection) throw new Error(`${store.key}: orders connection missing`);
...
module.exports = async function salesHistorySync(req, res) {
  ...
  try {
    const days = Math.min(Math.max(Number(req.body?.days || 90), 30), 90), startDate = new Date(Date.now() - days * 86400000).toISOString().slice(0,10);
    ...
    const results = []; for (const store of stores) results.push({ store, ...(await collectSales(store, startDate)) });
    ...
    const purge = await fetch(`${url}/rest/v1/shopify_sales_daily?sales_date=gte.${startDate}`, {method:'DELETE',headers,signal:AbortSignal.timeout(15000)}); if(!purge.ok) throw new Error('Could not refresh sales mirror');
    ...
    return res.status(200).json({ok:true,days,orders:results.reduce((n,x)=>n+x.orders,0),lines:results.reduce((n,x)=>n+x.lines,0),mirrored:rows.length,skipped});
