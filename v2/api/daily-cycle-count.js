...
async function managedLocations(url,key,userId){
  const r=await fetch(url+'/rest/v1/user_location_access?user_id=eq.'+encodeURIComponent(userId)+'&can_manage=eq.true&select=location_id,locations(id,name,active)',{headers:jsonHeaders(key),signal:AbortSignal.timeout(8000)});
...
async function openRun(url,key,locationId,user){
  const r=await fetch(url+'/rest/v1/rpc/open_v2_daily_cycle_count',{method:'POST',headers:{...jsonHeaders(key),'Content-Type':'application/json'},body:JSON.stringify({p_location_id:locationId,p_business_date:today(),p_target_count:5,p_user_id:user.id,p_user_name:user.display_name}),signal:AbortSignal.timeout(10000)});
  const data=await r.json().catch(()=>({}));if(!r.ok)throw Error(data.message||data.error||'Could not start today’s cycle count.');return Number(data);
}
async function runPayload(url,key,runId){
  const r=await fetch(url+'/rest/v1/cycle_count_runs?id=eq.'+runId+'&select=id,location_id,business_date,status,target_count,submitted_at,locations(name),cycle_count_lines(id,product_id,counted_quantity,status,counted_at,note,products(sku,name,barcode))&limit=1',{headers:jsonHeaders(key),signal:AbortSignal.timeout(8000)});
  const rows=await r.json();if(!r.ok||!rows.length)throw Error(rows.message||'Cycle count not found.');
  const run=rows[0];run.lines=(run.cycle_count_lines||[]).sort((a,b)=>(a.products?.sku||'').localeCompare(b.products?.sku||''));delete run.cycle_count_lines;return run;
}
module.exports=async(req,res)=>{
  ...
  try{
    const {url,serviceRoleKey:key}=configuration(),locations=await managedLocations(url,key,auth.user.id);
    ...
    if(!locations.some(x=>x.id===requested))return res.status(403).json({ok:false,error:'You can count only at your assigned warehouse.'});
    const runId=await openRun(url,key,requested,auth.user);
    if(req.method==='GET')return res.json({ok:true,locations,run:await runPayload(url,key,runId)});
    ...
    if(all.lines.every(line=>line.status!=='pending')&&all.status==='open'){
      await fetch(url+'/rest/v1/cycle_count_runs?id=eq.'+runId,{method:'PATCH',headers:{...jsonHeaders(key),'Content-Type':'application/json','Prefer':'return=minimal'},body:JSON.stringify({status:'ready_for_review',submitted_at:new Date().toISOString()}),signal:AbortSignal.timeout(8000)});
    }
    const updated=await runPayload(url,key,runId);
    return res.json({ok:true,run:updated,result:{countedQuantity:counted,variance}});
  }catch(error){return res.status(500).json({ok:false,error:error.message||'daily_cycle_count_failed'});}
};