...
async function requireControl(url, key, flag) {
  const rows = await rest(url, key, `mfg_feature_flags?flag_key=eq.${encodeURIComponent(flag)}&enabled=eq.true&select=flag_key&limit=1`);
  if (!rows.length) throw new Error(`manufacturing_control_disabled:${flag}`);
}
