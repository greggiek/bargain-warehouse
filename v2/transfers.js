(() => {
  ...
  function renderDraftLines(){transferDialogLines.replaceChildren();if(!transferDraftLines.length)return empty(transferDialogLines,4,'No items added yet.');transferDraftLines.forEach((line,index)=>{const row=document.createElement('tr');cell(row,line.sku);const product=document.createElement('td'),parent=document.createElement('span');parent.className='po-product-title';parent.textContent=line.name||'—';product.append(parent);if(line.variantTitle){const child=document.createElement('span');child.className='po-variant-title';child.textContent=line.variantTitle;product.append(child);}row.append(product);cell(row,String(line.quantity));const actions=document.createElement('td'),remove=document.createElement('button');remove.type='button';remove.className='button secondary';remove.textContent='Remove';remove.addEventListener('click',()=>{transferDraftLines.splice(index,1);renderDraftLines();});actions.append(remove);row.append(actions);transferDialogLines.append(row);});}
