(() => {
  ...
  function openTransferDetails(link, editing=false){transferDetailsTitle.textContent='Transfer '+link.bm_reference;transferDetailsBody.replaceChildren();const editable=link.status==='draft'&&editing;const route=document.createElement('p');route.className='muted';route.textContent=(link.source_name||'—')+' → '+(link.destination_name||'—')+' · '+formatStatus(link.status);const editActions=document.createElement('div');editActions.className='inventory-actions';if(link.status==='draft'&&!editable){const editButton=document.createElement('button');editButton.type='button';editButton.className='button';editButton.textContent='Edit draft';editButton.addEventListener('click',()=>openTransferDetails(link,true));editActions.append(editButton);}const wrap=document.createElement('div');wrap.className='inventory-table-wrap';const table=document.createElement('table');table.className='inventory-table';table.innerHTML='<thead><tr><th>SKU</th><th>Quantity</th></tr></thead><tbody></tbody>';const edits=[];(link.shopify_transfer_link_lines||[]).forEach(line=>{const row=document.createElement('tr');cell(row,line.sku||'—');const quantityCell=document.createElement('td');if(editable){const input=document.createElement('input');input.className='inventory-search';input.type='number';input.min='1';input.step='1';input.value=String(line.quantity||1);input.style.width='90px';edits.push({lineId:line.id,input});quantityCell.append(input);}else quantityCell.textContent=String(line.quantity||0);row.append(quantityCell);table.querySelector('tbody').append(row);});wrap.append(table);transferDetailsBody.append(route,editActions,wrap);if(editable){const actions=document.createElement('div');actions.className='inventory-actions';actions.style.marginTop='16px';const save=document.createElement('button');save.type='button';save.className='button';save.textContent='Save draft';save.addEventListener('click',async()=>{const lines=edits.map(item=>({lineId:item.lineId,quantity:Number(item.input.value)}));if(lines.some(line=>!Number.isInteger(line.quantity)||line.quantity<=0))return alert('Every quantity must be a whole number above zero.');save.disabled=true;try{const response=await fetch('/api/shopify-transfer-lifecycle',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({action:'edit_draft',linkId:link.id,lines})});const data=await response.json();if(!response.ok)throw new Error(data.error||'Draft could not be updated.');show(data.message);await loadShopifyTransfers();hideDialog(transferDetailsDialog);}catch(error){alert(error.message||'Draft could not be updated.');}finally{save.disabled=false;}});const remove=document.createElement('button');remove.type='button';remove.className='button secondary';remove.textContent='Delete draft';remove.style.color='#b44747';remove.addEventListener('click',async()=>{if(!confirm('Delete draft '+link.bm_reference+'? This cannot be undone. No inventory has moved.'))return;remove.disabled=true;try{const response=await fetch('/api/shopify-transfer-lifecycle',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({action:'delete_draft',linkId:link.id})});const data=await response.json();if(!response.ok)throw new Error(data.error||'Draft could not be deleted.');show(data.message);hideDialog(transferDetailsDialog);await loadShopifyTransfers();}catch(error){alert(error.message||'Draft could not be deleted.');}finally{remove.disabled=false;}});actions.append(save,remove);transferDetailsBody.append(actions);}else if(link.status==='pending'&&link.metadata?.outbound_status!=='shipped'&&shopifyTransferCapabilities.canShip){const actions=document.createElement('div');actions.className='inventory-actions';actions.style.marginTop='16px';const edit=statusButton('Return to draft to edit',link,'return_to_draft');actions.append(edit);transferDetailsBody.append(actions);}showDialog(transferDetailsDialog);}
  ...
  transferCreateDialog.className='transfer-workspace-dialog';
  transferCreateDialog.innerHTML='<form method="dialog" class="transfer-workspace"><header class="transfer-workspace-head"><button class="button secondary" type="submit">Cancel</button><div><div class="transfer-kicker">Transfer</div><h2>Create transfer</h2><p class="muted">Move one or more SKUs between locations.</p></div></header><section class="transfer-form-section"><h3>Transfer details</h3><div class="transfer-details-grid"><label>Transfer from<select id="shopifyTransferFrom" class="inventory-search" aria-label="From location"></select></label><label>Receiving location<select id="shopifyTransferTo" class="inventory-search" aria-label="To location"></select></label></div></section><section class="transfer-form-section"><h3>Add products</h3><div class="transfer-product-picker"><label class="transfer-product-search">Search product<div class="product-lookup"><input id="shopifyTransferSku" class="inventory-search" autocomplete="off" placeholder="Search SKU, barcode, or product"><div id="shopifyTransferSuggestions" class="product-suggestions" hidden></div></div></label><label>Category<select id="shopifyTransferCategory" class="inventory-search"><option value="">All categories</option></select></label></div><div class="transfer-line-entry"><label>Quantity<input id="shopifyTransferQuantity" class="inventory-search" type="number" min="1" step="1" placeholder="Qty"></label><button id="shopifyTransferAddLine" class="button secondary" type="button">Add item</button></div><div class="inventory-table-wrap transfer-draft-table"><table class="inventory-table"><thead><tr><th>SKU</th><th>Product</th><th>Quantity</th><th>Action</th></tr></thead><tbody id="shopifyTransferDraftLines"></tbody></table></div></section><footer class="transfer-action-bar"><p id="shopifyTransferCreateStatus" class="inventory-status">Choose the route, add the SKUs, then create the draft.</p><div><button class="button secondary" type="submit">Cancel</button><button id="shopifyTransferCreateDraft" class="button" type="button">Create draft</button></div></footer></form>';
  document.body.append(transferCreateDialog);
  const transferDialogFrom=transferCreateDialog.querySelector('#shopifyTransferFrom'),transferDialogTo=transferCreateDialog.querySelector('#shopifyTransferTo'),transferDialogSku=transferCreateDialog.querySelector('#shopifyTransferSku'),transferDialogCategory=transferCreateDialog.querySelector('#shopifyTransferCategory'),transferDialogQty=transferCreateDialog.querySelector('#shopifyTransferQuantity'),transferDialogSuggestions=transferCreateDialog.querySelector('#shopifyTransferSuggestions'),transferDialogLines=transferCreateDialog.querySelector('#shopifyTransferDraftLines'),transferDialogStatus=transferCreateDialog.querySelector('#shopifyTransferCreateStatus'),transferDialogAdd=transferCreateDialog.querySelector('#shopifyTransferAddLine'),transferDialogCreate=transferCreateDialog.querySelector('#shopifyTransferCreateDraft');
  let transferDraftLines=[],dialogSearchTimer,dialogSearchRequest=0,selectedTransferProduct=null;
  ...
  function renderDraftLines(){transferDialogLines.replaceChildren();if(!transferDraftLines.length)return empty(transferDialogLines,4,'No items added yet.');transferDraftLines.forEach((line,index)=>{const row=document.createElement('tr');cell(row,line.sku);const product=document.createElement('td'),parent=document.createElement('span');parent.className='po-product-title';parent.textContent=line.name||'—';product.append(parent);if(line.variantTitle){const child=document.createElement('span');child.className='po-variant-title';child.textContent=line.variantTitle;product.append(child);}row.append(product);const quantityCell=document.createElement('td'),quantityInput=document.createElement('input');quantityInput.type='number';quantityInput.min='1';quantityInput.step='1';quantityInput.value=String(line.quantity);quantityInput.className='line-quantity-input';quantityInput.setAttribute('aria-label',line.sku+' quantity');quantityInput.addEventListener('change',()=>{const value=Number(quantityInput.value);if(!Number.isInteger(value)||value<1){quantityInput.value=String(line.quantity);return;}line.quantity=value;});quantityCell.append(quantityInput);row.append(quantityCell);const actions=document.createElement('td'),remove=document.createElement('button');remove.type='button';remove.className='button secondary';remove.textContent='Remove';remove.addEventListener('click',()=>{transferDraftLines.splice(index,1);renderDraftLines();});actions.append(remove);row.append(actions);transferDialogLines.append(row);});}
  function fillTransferCategories(categories){const current=transferDialogCategory.value;transferDialogCategory.replaceChildren(new Option('All categories',''));[...new Set((categories||[]).map(item=>String(item||'').trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b)).forEach(category=>transferDialogCategory.add(new Option(category,category)));transferDialogCategory.value=current;}
  async function loadTransferCategories(){try{const response=await fetch('/api/transfers?productCategories=1',{credentials:'same-origin'});const data=await response.json();if(response.ok)fillTransferCategories(data.categories||[]);}catch{}}
  function hideDialogSuggestions(){transferDialogSuggestions.replaceChildren();transferDialogSuggestions.hidden=true;}
  function showDialogSuggestions(products){transferDialogSuggestions.replaceChildren();const category=transferDialogCategory.value;if(category)products=products.filter(product=>String(product.category||'')===category);if(!products.length)return hideDialogSuggestions();products.forEach(product=>{const option=document.createElement('button');option.type='button';option.className='product-suggestion';option.innerHTML='<strong></strong><small></small>';option.querySelector('strong').textContent=product.sku+' — '+product.name;option.querySelector('small').textContent=[product.variant_title||'',product.barcode?'Barcode: '+product.barcode:'',product.category||''].filter(Boolean).join(' · ');option.addEventListener('click',()=>{const existing=transferDraftLines.find(line=>line.sku.toLowerCase()===String(product.sku||'').toLowerCase());if(existing)existing.quantity+=1;else transferDraftLines.push({sku:product.sku,quantity:1,name:product.name||'',variantTitle:product.variant_title||''});selectedTransferProduct=null;transferDialogSku.value='';hideDialogSuggestions();renderDraftLines();dialogMessage(transferDraftLines.length+' SKU '+(transferDraftLines.length===1?'line':'lines')+' ready for Shopify preview.');});transferDialogSuggestions.append(option);});transferDialogSuggestions.hidden=false;}
  async function searchDialogProducts(){const term=transferDialogSku.value.trim();selectedTransferProduct=null;if(term.length<2)return hideDialogSuggestions();const request=++dialogSearchRequest;try{const response=await fetch('/api/transfers?productSearch='+encodeURIComponent(term),{credentials:'same-origin'});const data=await response.json();if(request!==dialogSearchRequest||!response.ok)throw new Error(data.error||'Lookup failed');showDialogSuggestions(data.products||[]);}catch(error){hideDialogSuggestions();dialogMessage(error.message||'Product lookup is unavailable.',true);}}
  function addDraftLine(){const lineSku=transferDialogSku.value.trim(),lineQuantity=Number(transferDialogQty.value);if(!lineSku||!Number.isInteger(lineQuantity)||lineQuantity<=0)return dialogMessage('Enter an exact SKU and whole-piece quantity.',true);if(transferDraftLines.some(line=>line.sku.toLowerCase()===lineSku.toLowerCase()))return dialogMessage('Each SKU may be included once. Remove the line and add it again if the quantity changed.',true);if(transferDraftLines.length>=50)return dialogMessage('A transfer can include up to 50 SKU lines.',true);transferDraftLines.push({sku:lineSku,quantity:lineQuantity,name:selectedTransferProduct?.name||'',variantTitle:selectedTransferProduct?.variant_title||''});selectedTransferProduct=null;transferDialogSku.value='';transferDialogQty.value='';hideDialogSuggestions();renderDraftLines();dialogMessage(transferDraftLines.length+' SKU '+(transferDraftLines.length===1?'line':'lines')+' ready for Shopify preview.');transferDialogSku.focus();}
  function openShopifyTransferDialog(){transferDialogFrom.replaceChildren(...[...from.options].map(option=>option.cloneNode(true)));transferDialogTo.replaceChildren(...[...to.options].map(option=>option.cloneNode(true)));transferDialogFrom.value=from.value;transferDialogTo.value=to.value;transferDraftLines=[];transferDialogSku.value='';transferDialogQty.value='';renderDraftLines();loadTransferCategories();dialogMessage('Add one or more SKUs, then create the Shopify draft.');showDialog(transferCreateDialog);transferDialogFrom.focus();}
  ...
  transferDialogCategory.addEventListener('change',searchDialogProducts);
