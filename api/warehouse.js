...
async function manageTransfer(req,res,session){
  ...
  const patch={notes:String(body.notes||'').trim()||null,updated_at:new Date().toISOString()};if(body.fromLocationId)patch.from_location_id=Number(body.fromLocationId);if(body.toLocationId)patch.to_location_id=Number(body.toLocationId);if(patch.from_location_id&&patch.to_location_id&&patch.from_location_id===patch.to_location_id)return res.status(400).json({ok:false,error:'Source and destination must be different.'});
