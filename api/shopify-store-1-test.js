module.exports = async function (req, res) {
  const shop = (process.env.SHOPIFY_STORE_1_DOMAIN || '')
    .replace(/^https?:\/\//, '')
    .replace(/\/$/, '');

  const clientId = process.env.SHOPIFY_STORE_1_CLIENT_ID;
  const clientSecret = process.env.SHOPIFY_STORE_1_CLIENT_SECRET;

  if (!shop || !clientId || !clientSecret) {
    return res.status(500).json({
      ok: false,
      error:
        'Missing SHOPIFY_STORE_1_DOMAIN, SHOPIFY_STORE_1_CLIENT_ID, or SHOPIFY_STORE_1_CLIENT_SECRET'
    });
  }
