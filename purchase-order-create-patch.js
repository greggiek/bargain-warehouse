(() => {
  ...
  async function renderCreatePurchaseOrder() {
    ...
    try {
      ...
      app.innerHTML = `<section class="panel po-create"><div class="section-head"><div><div class="eyebrow">RECEIVING</div><h2>New Purchase Order</h2></div><button id="cancelPo" class="secondary">Back</button></div>
        ...
        <div id="poLines"></div><div id="poTotal" class="po-total">Total: $0.00</div>
  ...
  function updateTotal(){const total=collectLines().reduce((s,l)=>s+(l.orderedQty||0)*(l.unitCost||0),0);const el=document.getElementById('poTotal');if(el)el.textContent=`Total: $${total.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}`}
