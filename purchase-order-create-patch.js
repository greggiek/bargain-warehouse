(() => {
  ...
  async function loadPurchaseOrderMaster(){
    ...
    try{
      ...
      const response=await fetch('/api/warehouse?action=master-pos',{cache:'no-store',headers:{Authorization:`Bearer ${token}`}}),data=await response.json();if(!response.ok)throw new Error(data.error||'Could not load purchase orders.');
